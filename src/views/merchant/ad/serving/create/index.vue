        ...
        methods: {
            ...
            handleGetAdType () {
                getAdType().then(res => {
                    ...
                    if (res.result == 1) {
                        // 深拷贝并添加属性disabled
                        this.form.adOptions = this.myAdOptions(res.data)
                    }
                })
            },
            myAdOptions (obj) {
                        ...
                        for (let key in obj) {
                            if (obj.hasOwnProperty(key)) {
                                ...
                                if(obj[key] && typeof obj[key] === "object") {
                                    ...
                                    if(key === 'list' && obj[key].length === 0) {
                                        objClone['disabled'] = true
                                    }
                                    objClone[key] = this.myAdOptions(obj[key])
                                } else {
                                    //如果不是，简单复制
            ...
            handleAdCascaderVisible (boolean) {
                // console.log(this.targetId, this.targetType)
                if (!boolean) {
                    if (this.adSelectedOptions.length) {
                        this.handleGetCityBlock()
                        this.getSttIdMehtod()
                    }
            ...
            addAreaTag () {
                ...
                if (JSON.stringify(this.areaTagsCache) == '{}') {
                    this.$message({type: 'warning', message: '请选择地区再添加'})
                    return false
                }
                // console.log(this.form.areaTags)
